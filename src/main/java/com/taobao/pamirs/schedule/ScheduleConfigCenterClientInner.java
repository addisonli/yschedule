...
public List<ScheduleTaskTypeRunningInfo> getAllTaskTypeRunningInfo(Connection conn,String baseTaskType) throws Exception {
	...
	public List<ScheduleServer> selectAllValidScheduleServer(Connection conn,String taskType)
		...
		List<ScheduleServer> result = new ArrayList<ScheduleServer>();
			String sql = " SELECT UUID,TASK_TYPE,BASE_TASK_TYPE,OWN_SIGN,IP,HOST_NAME,MANAGER_PORT," +
					"THREAD_NUM,REGISTER_TIME,HEARTBEAT_TIME,LAST_FETCH_DATA_TIME,NEXT_RUN_START_TIME,NEXT_RUN_END_TIME,VERSION,DEALINFO_DESC,JMX_URL, "
			...
			while (resultSet.next()) {
				...
				server.setLastFetchDataTime(resultSet.getTimestamp("LAST_FETCH_DATA_TIME"));
	...
	public void refreshScheduleServer(Connection conn,ScheduleServer server) throws Exception {
			String REFRESH_SQL_UPDATE = " UPDATE "
					...
					+ " LAST_FETCH_DATA_TIME =?,NEXT_RUN_START_TIME =?,NEXT_RUN_END_TIME =?,GMT_MODIFIED = "
			...
			statement.setTimestamp(2, server.getLastFetchDataTime());
			statement.setString(3, server.getNextRunStartTime());
			statement.setString(4, server.getNextRunEndTime());
			statement.setString(5, server.getUuid());
	...
	public List<ScheduleServer> selectScheduleServer(Connection conn,boolean isHistoryData,
			...
			throws Exception {
		List<ScheduleServer> result = new ArrayList<ScheduleServer>();
		String sql = " SELECT UUID,TASK_TYPE,BASE_TASK_TYPE,OWN_SIGN,IP,HOST_NAME,MANAGER_PORT,THREAD_NUM,REGISTER_TIME,HEARTBEAT_TIME,LAST_FETCH_DATA_TIME,NEXT_RUN_START_TIME,NEXT_RUN_END_TIME,VERSION,DEALINFO_DESC,JMX_URL, "
				+ this.getDataBaseSysdateString(conn)
				+ " as CenterServerTime   FROM  ";
